...
class PersistentContextEngine:
    ...
    def _find_similar(self, canonical: str, ts: datetime, current_iid: str,
        ...
        for iid, mem in self._incidents.items():
            ...
            key = mem.family
            if key not in best_per_family or score > best_per_family[key][0]:
                best_per_family[key] = (score, same_can, mem)

        # Force exact family first
        items = list(best_per_family.values())
        exact = [x for x in items if x[2].family == query_family]
        rest = [x for x in items if x[2].family != query_family]
        ...
        result = []
        for score, same_can, mem in final[:5]:
            rationale = []
